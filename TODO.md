...
- [x] **User-need catalog module.** New file (e.g. `src/domain/needCatalog.ts`) holding the preset list of user-need options for Phase 1's dropdown, shaped as `{id, label}` (or similar) so Phase 3's question bank can reuse the same shape later.
- [x] **Replace the static demo with a `Scenario`.** Restructure `src/demos/userNeedDependency.ts` into a `Scenario`: an ordered list of steps, each one rendering the Panel, waiting for visitor input/confirm, mutating the domain, then celebrating and advancing. Move the toolbox-toggling logic currently duplicated in `index.html`'s and `preview.html`'s `<script>` blocks into this shared Scenario code, so both host pages drive one source of truth instead of two copies.
